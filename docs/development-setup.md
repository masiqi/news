...
本指南将帮助您启动墨香蒸馏的所有组件，包括前端、后端和管理后台。
...
- 提供更清晰的启动信息
- 简化开发流程
- 监听所有网络接口（0.0.0.0），支持远程访问

### 网络访问配置

默认情况下，所有服务都监听0.0.0.0地址，这意味着可以从网络上的任何IP地址访问这些服务。如果您只想在本地访问，可以在相应的`start.sh`脚本中修改主机地址配置。

#### 服务访问地址：
- **前端服务**: http://your-server-ip:3000
- **后端API服务**: http://your-server-ip:8787
- **管理后台**: http://your-server-ip:8000

其中`your-server-ip`是您的Ubuntu服务器的IP地址。
