...
- [ ] 项目结构符合架构文档中定义的规范：`frontend` (Next.js), `backend` (Hono.js), `packages/shared`, `packages/db` (Drizzle ORM)
...
- [ ] Task 1: 初始化Monorepo项目结构 (AC: 1, 6)
  ...
  - [ ] 创建packages/shared和packages/db目录
- [ ] Task 2: 配置前端应用 (AC: 4)
...
- **包组织**: `frontend` (Next.js), `backend` (Hono.js), `packages/shared`, `packages/db` (Drizzle ORM) [Source: architecture.md#2.3]
...
- packages/db: 数据库相关代码和Drizzle ORM配置
