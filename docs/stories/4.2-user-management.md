...
## Acceptance Criteria
- [ ] 创建用户列表页面，显示所有用户的基本信息
- [ ] 实现用户搜索和筛选功能（按用户名、邮箱、注册时间等）
- [ ] 提供用户详细信息查看功能
- [ ] 支持用户状态管理（启用/禁用账户）
- [ ] 实现用户角色和权限管理
- [ ] 提供用户操作日志查看功能
...
## Tasks / Subtasks
- [ ] Task 1: 设计用户管理数据模型 (AC: 4, 5)
  - [ ] 扩展用户表，添加管理相关字段
  - [ ] 创建用户角色和权限模型
  - [ ] 设计用户操作日志表结构
  - [ ] 实现用户状态和角色管理逻辑
- [ ] Task 2: 实现用户管理API (AC: 1, 3, 4, 5, 7)
  - [ ] 创建用户查询和搜索API
  - [ ] 实现用户详情查看API
  - [ ] 添加用户状态管理API
  - [ ] 实现角色和权限管理API
...
- `/backend/src/routes/admin/users.ts` (待创建)
- `/backend/src/routes/admin/roles.ts` (待创建)
- `/backend/src/services/admin/user-management.service.ts` (待创建)
- `/backend/src/services/admin/role-permission.service.ts` (待创建)
- `/backend/src/services/admin/audit-log.service.ts` (待创建)
- `/admin/src/pages/users/list.tsx` (待创建)
- `/admin/src/pages/users/detail.tsx` (待创建)
- `/admin/src/components/users/UserTable.tsx` (待创建)
- `/admin/src/components/users/UserStatusManager.tsx` (待创建)
- `/admin/src/components/users/RoleManager.tsx` (待创建)
